...
const Dashboard: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (params.toolMode === 'modify') {
        ...
        setIsLoading(true);
        try {
            console.log("Initializing Gemini Service for Modify...");
            ...
            console.error("MODIFICATION ERROR DETAILS:", error);
            alert(`Modification failed: ${error.message || "Unknown error"}`); 
        } 
    ...
    setIsLoading(true);
    try {
      console.log("Initializing Gemini Service for Create...");
      const { GeminiService } = await import('./services/geminiService');
      const gemini = new GeminiService();
